  ...
  if [ -d $KLIPPER_DIR/.git ]; then
    ...
    if [ "$(echo $GET_BRANCH | grep "HEAD" )" ]; then
      ...
      if [[ $GET_BRANCH =~ [[:alnum:]] ]]; then
        ...
        elif [ "$(git branch -r --contains $GET_BRANCH | grep "dgus-display")" ]; then
          GET_BRANCH="dgus-display"
        fi
      fi
    fi
  else
...
read_remote_klipper_commit(){
  ...
  if [ ! -z "$GET_BRANCH" ];then
    ...
    elif [ "$GET_BRANCH" = "desuuuu/dgus-display" ]; then
      git fetch desuuuu master -q
      REMOTE_COMMIT=$(git describe desuuuu/master --always --tags | cut -d "-" -f 1,2)
    fi
  else
    REMOTE_COMMIT=$NONE
  fi
