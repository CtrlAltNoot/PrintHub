  ...
  source_kiauh_ini
    ...
    if ls $klipper_cfg_loc/printer_* 2>/dev/null 1>&2; then
      for config in $(find $klipper_cfg_loc/printer_*/printer.cfg); do
        ...
        if [ ! -d $path/macros/ ]; then
          ### copy kiauh_macros.cfg to config location
          status_msg "generating macros folder and files ..."
          rsync -a ${SRCDIR}/kiauh/resources/macros/ $path/macros/
        fi
      done
    ### handle single printer.cfg
    elif [ -f $klipper_cfg_loc/printer.cfg ] && [ ! -d $klipper_cfg_loc/macros/ ]; then
      ### copy kiauh_macros.cfg to config location
      status_msg "Creating macro config file ..."
      rsync -a ${SRCDIR}/kiauh/resources/macros/ $klipper_cfg_loc/macros/
    fi
